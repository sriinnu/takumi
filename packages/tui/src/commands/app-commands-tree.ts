...
import type { FlatTreeEntry, SessionData, SessionTreeManifest, SessionTreeNode } from "@takumi/core";
import {
	branchSession,
	flattenTree,
	getAncestors,
...
async function restoreLoadedSession(ctx: AppCommandContext, session: SessionData, notice: string): Promise<void> {
	if (ctx.activateSession) {
		await ctx.activateSession(session, notice, "resume");
		return;
	}
	...
	ctx.state.turnCount.value = countSessionTurns(session.messages);
	ctx.agentRunner?.hydrateHistory(session.messages);
	ctx.addInfoMessage(notice);
}
...
export function registerSessionTreeCommands(ctx: AppCommandContext): void {
	...
	ctx.commands.register(
		...
		async () => {
			...
			await restoreLoadedSession(ctx, parent, `Navigated to parent: ${parent.title || parent.id}`);
		},
		["/up"],
	);
}
