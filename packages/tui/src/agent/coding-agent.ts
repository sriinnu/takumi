...
} from "../chitragupta/chitragupta-executor-runtime.js";
import {
...
import { buildResumeCompatibilitySummary } from "./coding-agent-resume.js";
import { resolveRoutingOverrides } from "./coding-agent-routing.js";
import { recordSabhaExecutorOutcome } from "./coding-agent-sabha.js";
...
export class CodingAgent {
	...
	async cancel(reason = "Task cancelled."): Promise<void> {
		if (!this.isActive && !this.orchestrator) return;
	...
	async start(description: string, forceMode?: "single" | "multi"): Promise<void> {
		...
		let routedLaneEnvelopes = {} as Awaited<ReturnType<typeof resolveRoutingOverrides>>["laneEnvelopes"];
		...
		if (forceMode) {
			...
			try {
				...
				const routingPlan = await resolveRoutingOverrides({
					...
					emitRouteEvent: this.runner.emitExtensionEvent,
				});
				this.state.routingDecisions.value = appendRoutingDecisions(
					this.state.routingDecisions.value,
					routingPlan.decisions,
				);
				routedLaneEnvelopes = routingPlan.laneEnvelopes;
				for (const decision of routingPlan.decisions)
					if (decision.degraded) recordRouteDegradedExecution(this.state, decision);
		...
		if (orchestrationMode === "multi" && this.orchestrator) {
			...
			const cfg = prepareMeshCluster({
				...
				laneEnvelopes: routedLaneEnvelopes,
			});
		...
		try {
			...
			await recordSabhaExecutorOutcome(this.state, boundSessionId, 0.85);
			...
			await recordSabhaExecutorOutcome(this.state, boundSessionId, 0.35);
	...
	async resume(clusterId: string): Promise<void> {
		...
		this.orchestrator.setChitraguptaMemory(this.state.chitraguptaMemory.value || undefined);
		let state: Awaited<ReturnType<ClusterOrchestrator["resume"]>>;
		try {
			state = await this.orchestrator.resume(clusterId);
		} catch (err) {
			const summary = buildResumeCompatibilitySummary(this.orchestrator);
			if (summary) {
		...
		const summary = buildResumeCompatibilitySummary(this.orchestrator);
		if (summary) this.addSystemMessage(summary);
	...
	async shutdown(): Promise<void> {
		...
		if (this.isActive && !this.stopRequestedReason) await this.cancel("Coding task shutdown requested.");
...
// Resume and Sabha helpers live beside this class so the coding agent stays within the LOC guardrail.
