...
import { refreshTrackedNativeSideAgentLane } from "./workflow-side-agent-lanes.js";
...
export function registerSideLaneCommands(ctx: AppCommandContext): void {
	...
	ctx.commands.register("/lane-send", "Send a prompt to a tracked side lane", async (args) => {
	...
	ctx.commands.register("/lane-stop", "Stop a tracked side lane and release its resources", async (args) => {
