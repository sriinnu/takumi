...
export class KeyBindingRegistry {
	...
	register(key: string, description: string, handler: () => void, options: RegisterKeyBindingOptions = {}): void {
		...
		const displaced = new Set<KeyBinding>();
		const addDisplaced = (binding: KeyBinding | undefined) => {
			if (binding) {
				displaced.add(binding);
			}
		};

		addDisplaced(this.bindingsById.get(id));
		addDisplaced(this.bindings.get(normalizedKey));
		for (const alias of aliases) {
			addDisplaced(this.bindings.get(alias));
		}

		for (const binding of displaced) {
