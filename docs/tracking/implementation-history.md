# Takumi (匠) — Implementation History

> **Archive note:** this file contains historical implementation planning and completion history.
>
> For the maintained tracking docs and feature residency model, start with:
>
> - `docs/tracking/README.md`
> - `docs/tracking/future-roadmap.md`
> - `docs/takumi-extensibility-model.md`
> - `docs/pi-mono-lessons.md`
>
> Keep using this file as an archive/reference, but treat the docs above as the active forward plan.
...
## 🔬 Autoresearch-Inspired Autocycle Follow-ups

**Context:** Audited against `karpathy/autoresearch` (March 2026). Takumi already has the core keep/discard loop, fixed eval budgets, JSONL ledgers, and safe revert behavior. The remaining wins are research-org ergonomics and persistence.

**Ownership note:** `Autocycle` currently lives in Takumi. Target architecture is for bounded research workflow ownership to move into Chitragupta via Prana, with Takumi acting as the operator/client surface for invocation, monitoring, interruption, and rendering.

- [x] Add Markdown experiment manifest for Autocycle runs (`.takumi/autocycle/*.experiment.md`)
- [x] Add TSV/column-based metric extraction fallback (`--metricColumn <name>`)
- [x] Add Chitragupta consolidation hook after each completed autocycle iteration
- [x] Add resumable autocycle runs from existing JSONL ledger state
- [ ] Add A/B branch mode for parallel experimental tracks with winner selection
- [ ] Add richer ledger fields for objective / hypothesis / mutation summary
- [ ] Add optional Git branch/tag discipline for long-running research loops
...
## Phase 13-19: Chitragupta Deep Integration ✅ COMPLETE
...
**Status:** 26/26 daemon RPC methods (100% coverage) ✅  
**Current:** All phases complete (13-19)
...
### Phase 19: Session Recovery & Replay ✅ COMPLETE
...
**See Also:** `docs/tracking/future-roadmap.md` and `docs/pi-mono-lessons.md` — maintained roadmap and durable Pi-derived guidance

### Phase 20: Telemetry & Observability ✅ COMPLETE
...
### Phase 21: Side Agent Integration ✅ COMPLETE
...
### Phase 22: HTTP Bridge & Remote Access ✅ COMPLETE
...
## Productization Track (Post-Phase 54)

These are the **accepted highest-value product additions next**. They are intentionally tracked as a productization lane rather than pretending they are just more core engine phases.

### P-Track 1: Approvals + Audit Trail ✅ COMPLETE
- [x] Add approval queue and operator review surface (`/approvals`, pending decisions, escalation path).
- [x] Persist approval records with actor, timestamp, tool, args summary, lane, and reason.
- [x] Add JSONL + CSV audit export.
- [x] Cover denial, escalation, replay, and session restore behavior with tests.

### P-Track 2: Eval / Regression Gate ✅ COMPLETE
- [x] Create benchmark corpus for core coding-agent tasks (plan, bug-fix, refactor, review, validation).
- [x] Record baseline metrics: success rate, cost, latency, retries, approvals, degraded lanes.
- [x] Add CI / local gate command to fail on unacceptable regression.
- [x] Emit machine-readable benchmark reports for release decisions.

### P-Track 3: Artifact Persistence + Handoff UX ✅ COMPLETE
- [x] Persist artifact payloads durably behind existing artifact types.
- [x] Add artifact retrieval by session, task, type, and time.
- [x] Build artifact browser/export flow in TUI + desktop shell.
- [x] Add structured handoff/reattach flow between sessions, branches, and side agents.

### P-Track 4: Packaging / Distribution ✅ COMPLETE
- [x] Define supported startup/install matrix for macOS, Linux, Windows, and WSL.
- [x] Harden terminal-first install/start docs and shell selection guidance.
- [x] Package desktop shell with a documented release path.
- [x] Keep the product surface explicit: terminal runtime, desktop companion, headless bridge.
- [x] Define and ship **Takumi Build Window** as the named desktop/operator shell surface.
- [x] Ensure Build Window can attach to terminal-first sessions running in Ghostty/tmux and to Windows/WSL-started sessions via the bridge.
- [x] Implement Build Window v1 slices from `docs/build-window-v1.md` starting with attachable operator shell.
- [x] Start Build Window Slice 1 as a **macOS + terminal + tmux + Chitragupta daemon-first** operator shell that uses daemon session/routing/health state as its source of truth.

### P-Track 5: Operator Observability ✅ COMPLETE
- [x] Add per-session and fleet summaries (status, cost, context pressure, failures).
- [x] Surface degraded routing, fallback chains, and approval pressure.
- [x] Add threshold alerts for context, cost spikes, and repeated failures.
- [x] Add replay/debug views for recent degraded runs.
